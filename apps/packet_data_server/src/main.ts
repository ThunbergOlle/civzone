...
import all_packet_handler, {
  sync_packet_handler,
} from "./packet/packet_handler";
...
redisPubClient.on("error", (err) =>
  log(err, LogLevel.ERROR, LogApp.PACKET_DATA_SERVER)
);
...
redisSubClient.pSubscribe(
  `${RedisPacketPublisher.channel_prefix}*`,
  (message, channel) => {
    const deconstructed_packet = DeconstructRedisPacket<unknown>(
      message,
      channel
    );
...
async function exit() {
  ...
  log(
    "Redis clients closed and worlds cleaned up.",
    LogLevel.INFO,
