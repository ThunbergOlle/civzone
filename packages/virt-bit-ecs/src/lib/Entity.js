 ...
 *
 * @param {World} world
 * @param {number} desiredEid - optional. If not provided, the eid will be auto generated.
 * @returns {number} eid
 */
export const addEntity = (world, desiredEid = null) => {
  let eid = desiredEid === null ? -1 : desiredEid

  if (eid === -1) {
    /* Auto generate entity id */
    if(world[$manualEntityRecycling]){
      eid = removed.length ? removed.shift() : globalEntityCursor++
    }else {
      /* This is the default case */
      eid = removed.length > Math.round(globalSize * removedReuseThreshold) ? removed.shift() : globalEntityCursor++
    }
  }else {
    // Check if eid is allready in use
    if (world[$entitySparseSet].has(eid)) throw new Error("bitECS - eid is already in use")

    /* Update global entity cursor */
    /* Make sure to skip the entities (remove) the ones which are in between. */
    if (eid > globalEntityCursor) {
      removed.push(...Array.from({ length: eid - globalEntityCursor }, (_, i) => i + globalEntityCursor)) // make entities that were "skiped" recyclable
    }
    globalEntityCursor = Math.max(eid + 1, globalEntityCursor)
  }
