...
export async function addToInventory(
  inventorySlots: UserInventoryItem[],
  itemId: number,
  quantity: number,
  slot: number
): Promise<number> {
  ...
  const similiar_inventory_stack = inventorySlots.filter(
    (i) => i.item && i.item.id === itemId
  );
  const empty_inventory_slots = inventorySlots.filter((i) => i.item === null);
  const stack_with_space = similiar_inventory_stack.find(
    (i) => i.quantity < stack_size
  );
  ...
  if (stack_with_space) {
    const new_quantity = Math.min(
      stack_with_space.quantity + quantity,
      stack_size
    );
    const remainder = Math.max(
      quantity - (stack_size - stack_with_space.quantity),
      0
    );
...
async function createNewStack(
  empty_inventory_slots: UserInventoryItem[],
  itemId: number,
  quantity: number,
  ...
  const slot =
    preferred_slot !== undefined
      ? empty_inventory_slots.find((s) => s.slot === preferred_slot)
      : empty_inventory_slots[0];
...
async function handleNegativeQuantities(
  ...
  if (slot) {
    log(
      `Removing ${quantity} of item ${itemId} from slot ${slot}`,
      LogLevel.INFO
    );
    // check if slot is occupied
    const slot_item = inventorySlots.find(
      (i) => i.slot === slot && i.item?.id === itemId
    );
    if (!slot_item) {
      log(`Slot ${slot} cannot be found in inventory`, LogLevel.ERROR);
      return quantity;
  ...
  if (quantity_left_to_remove > 0) {
    log(
      `Could not remove ${quantity_left_to_remove} of item ${itemId} from inventory. Potential quantity leak`,
      LogLevel.ERROR
    );
  }
...
export async function safe_move_items_between_inventories(transaction: {
  ...
  const { fromId, toId, toSlot, fromSlot, itemId, quantity } = transaction;

  const fromInventory = UserInventoryItem.addToInventory(
    fromId as string,
    itemId,
    -quantity,
    fromSlot
  ...
  const toInventory = UserInventoryItem.addToInventory(
    toId as string,
    itemId,
    quantity,
    toSlot
  ...
  const [from_quantity_left, to_quantity_left] = await Promise.all([
    fromInventory,
    toInventory,
  ]);
  ...
  if (from_quantity_left !== 0 || to_quantity_left !== 0) {
    ...
    if (from_quantity_left < 0) {
      ...
      log(
        `Tried to move items from inventory ${fromId} that we did not have.`,
        LogLevel.ERROR
      );
      // we need to remove the items from the toInventory
      const refundFromInventory = UserInventoryItem.addToInventory(
        toId as string,
        itemId,
        from_quantity_left,
        toSlot
      ...
      log(
        `Refunding ${from_quantity_left} of item ${itemId} from inventory ${toId} to inventory ${fromId}`,
        LogLevel.INFO
      );
    ...
    if (to_quantity_left > 0) {
      ...
      const refundToInventory = UserInventoryItem.addToInventory(
        fromId as string,
        itemId,
        to_quantity_left,
        fromSlot
      ...
      log(
        `Tried to move items to inventory ${toId} that we did not have.`,
        LogLevel.ERROR
      );
    }
  }
